...
def calculate_kl_divergence_sum(reference_proportions, set_proportions):
    kl_divergence_sum = 0
    for key in reference_proportions.keys():
        ...
        if q == 0:
            if key in ('対戦風景', 'アイテム', 'ファイター'):
                ...
                kl_divergence_sum += p * np.log(2) #'対戦結果'が２個のときとほぼ同じ
            continue
...
# df1の全体に対してtopic_tagの割合を計算
reference_proportions = count_topic_tag_proportions(df1)

# Set列の各値ごとにcount_topic_tag_proportionsを適用し、KLダイバージェンスを計算
results = []

for set_value, group_df in df2.groupby('Set'):
    # 各Setのtopic_tag割合を計算
    set_proportions = count_topic_tag_proportions(group_df)
    
    # KLダイバージェンスを計算
    kl_divergence = calculate_kl_divergence_sum(reference_proportions, set_proportions)
